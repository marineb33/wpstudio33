...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="mkdf-mobile-video-image" ' . chillnews_mikado_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	
	$video_output .= '<div ' . chillnews_mikado_get_class_attribute($video_overlay_class) . chillnews_mikado_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="mkdf-video-wrap">';
	$video_output .= '<video class="mkdf-video" width="1920" height="800" '. chillnews_mikado_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . chillnews_mikado_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="<?php esc_html__("Video thumb","chillnews"); ?>" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
