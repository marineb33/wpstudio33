...
if(!function_exists('chillnews_mikado_get_title')) {
    /**
     * Loads title area HTML
     */
    function chillnews_mikado_get_title() {
        $id = chillnews_mikado_get_page_id();

        extract(chillnews_mikado_title_area_height());
        extract(chillnews_mikado_title_area_background());
        ...
        if(get_post_meta($id, "mkdf_show_title_area_meta", true) !== ""){
            ...
            $show_title_area = chillnews_mikado_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        $parameters = apply_filters('chillnews_mikado_title_area_height_params', $parameters);

        chillnews_mikado_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('chillnews_mikado_get_title_text')) {
    /**
     * Function that returns current page title text. Defines chillnews_mikado_title_text filter
     ...
     */
    function chillnews_mikado_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false) . esc_html__(' Tag', 'chillnews');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'chillnews') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search', 'chillnews');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
            if(chillnews_mikado_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = chillnews_mikado_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'chillnews');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(chillnews_mikado_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(chillnews_mikado_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','chillnews');
        }
        ...
        $title = apply_filters('chillnews_mikado_title_text', $title);

        return $title;
    }
}

if(!function_exists('chillnews_mikado_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see chillnews_mikado_get_title_text()
     */
    function chillnews_mikado_title_text() {
        echo chillnews_mikado_get_title_text();
    }
}

if(!function_exists('chillnews_mikado_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see chillnews_mikado_is_product_category()
     * @see get_search_query()
     */
    function chillnews_mikado_custom_breadcrumbs() {
        global $post, $wp_query;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = chillnews_mikado_get_page_id();
        $bread_style = "";
        ...
        $delimiter = "<span class='mkdf-delimiter' ".chillnews_mikado_get_inline_style($bread_style)."></span>"; // delimiter between crumbs
        $home = esc_html__('Home','chillnews'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="mkdf-current" '.chillnews_mikado_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.chillnews_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.chillnews_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.chillnews_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="mkdf-breadcrumbs"><div class="mkdf-breadcrumbs-inner"><a '.chillnews_mikado_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;
            ...
            if ( is_category()) {
                ...
                $output .= $before . esc_html__('Search', 'chillnews') . $after;

            } elseif ( is_day() ) {
                $output .= '<a '.chillnews_mikado_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a '.chillnews_mikado_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a '.chillnews_mikado_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a '.chillnews_mikado_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts Tagged ', 'chillnews') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles Posted by ', 'chillnews') . $authordata->display_name . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404', 'chillnews') . $after;
            } elseif ( chillnews_mikado_is_woocommerce_installed() && is_shop() ){
                global $woocommerce;
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'chillnews') . ' ' . get_query_var('paged') . ")" . $after;
            }
...
if(!function_exists('chillnews_mikado_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function chillnews_mikado_get_title_area_height() {
        ...
        } elseif(chillnews_mikado_options()->getOptionValue('title_area_height') !== '') {
            $title_height = chillnews_mikado_options()->getOptionValue('title_area_height');
        }

        return apply_filters('chillnews_mikado_title_area_height', $title_height);
    }
}

if(!function_exists('chillnews_mikado_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function chillnews_mikado_get_title_content_padding() {
        return apply_filters('chillnews_mikado_title_content_padding', 0);
    }
}


if(!function_exists('chillnews_mikado_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function chillnews_mikado_title_area_height() {
        $id = chillnews_mikado_get_page_id();
        $title_height_and_padding = array();
        $title_height          = chillnews_mikado_get_title_area_height();
        $header_height_padding = chillnews_mikado_get_title_content_padding();
        ...
        if(get_post_meta($id, "mkdf_title_area_background_image_responsive_meta", true) != "") {
            ...
            $is_img_responsive = chillnews_mikado_options()->getOptionValue('title_area_background_image_responsive');
        }
        ...
        if(get_post_meta($id, "mkdf_title_area_vertial_alignment_meta", true) !== '') {
            ...
            $title_vertical_alignment = chillnews_mikado_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('chillnews_mikado_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function chillnews_mikado_title_area_background() {
        $id = chillnews_mikado_get_page_id();
        $show_title_img = true;
        ...
        if(get_post_meta($id, "mkdf_title_area_background_color_meta", true) != ""){
            ...
            $background_color = chillnews_mikado_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(get_post_meta($id, "mkdf_title_area_background_image_meta", true) != ""){
            ...
            $background_image = chillnews_mikado_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = chillnews_mikado_get_image_dimensions($background_image);
