...
abstract class ListShortcode {
    ...
    public function vcMap() {
        if(function_exists('vc_map')) {
            vc_map( array(
                ...
                "params" => chillnews_mikado_get_shortcode_params($this->base)
            ) );
        }
    ...
    public function renderHolders($atts, $content = null) {
        ...
        $args = chillnews_mikado_get_shortcode_params_names(chillnews_mikado_get_shortcode_params($this->base));
        ...
        $html = '';
        $html .= '<div class="mkdf-bnl-holder '.$this->css_class.'-holder  '.esc_attr($params['holder_classes']).'" '.chillnews_mikado_get_inline_attrs($params['posts_data']).'>';
    ...
    public function generatePostsQuery($params) {
        $queryResult = chillnews_mikado_get_query($params);
    ...
    private function generateImageSize($params, $type) {
        ...
        if ($imageSize !== '' && $imageSize == 'landscape') {
            ...
            $thumbImageSize = 'chillnews_mikado_portrait';
    ...
    protected function errorMessage() {
        return '<div class="'.$this->css_class.'-message"><p>'.esc_html__('No posts were found.', 'chillnews').'</p></div>';
    }
    ...
    private function getPaginationHtml($params, $atts) {
        ...
        switch($params['pagination_type']){
            case 'load-more' : {
                $html .= chillnews_mikado_get_list_shortcode_module_template_part('post-show-load-more-template', 'templates', '', $atts);
            } break;
            case 'np-horizontal' : {
                $html .= chillnews_mikado_get_list_shortcode_module_template_part('post-direction-nav-horizontal-template', 'templates', '', $atts);
                $html .= chillnews_mikado_get_list_shortcode_module_template_part('post-ajax-preloader-template', 'templates', '', $atts);
            } break;
            default : break;
        }
